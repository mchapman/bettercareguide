...
## create models from a table with associated lookups
Given(/^the following #{capture_plural_factory} with association exists?:?$/) do |plural_factory, table|
  ...
  table.hashes.map do |hash|
    ...
    hash.each do |k, v|
      if k[0..1] == 'a_'
        split_key = k[2..-1].split('@')
        classname = split_key[0]
        lookup_fld = split_key[1]
        foreign_key_field = split_key[2] + '_id'
        eval_str = "#{classname}::find_by_#{lookup_fld}('#{v}').id"
        new_hash[foreign_key_field] = eval eval_str
      else
        new_hash[k] = v
      end
    end
    create_model(pickle_ref, new_hash)
  end
end
